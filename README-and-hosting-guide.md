...
- **Mesh size per cage type** — every cage style can carry its mesh size (⚙ → Manage cage types → add it when creating a type, or ✎ an existing one). It shows in the type list, on the cage sheet, and when you peek an empty cage ("Empty · Flip · 9 mm mesh").
- **Work-again reminders** — every growth check / mortality / harvest / stocking form has an optional "Work again" row (1, 2, 4, or 8 weeks, or pick a date). The red needs-work ring fires when that date passes — cages with a scheduled date won't ring early, and cages without one keep the 8-week rule. The peek and batch card show "work due ___" so you know before you're on the water.
...
3. **The app opens on the whole-farm overview**: every plot is a card showing its lines as rows of colored dots. Tap a plot to open it full-screen with big cages sized for a stylus on the water. **‹ Farm** (or the phone's back button) returns to the overview, and the chips across the top hop straight between plots without going back.
   - **The stats row** counts the whole farm: cages, stocked, **total oysters** (sum of every known count), average size, and market-ready. Tap **Market ready ▸** for the month-by-month outlook — how many cages (and roughly how many oysters) the growth model expects to hit market size each month over the next year, plus what's ready right now.
...
7. **Quick peek vs full sheet**: tap a cage once (or hover with a mouse) for a small popup with the essentials — size, when it went in, and count. Double-tap (or double-click) to open the full cage sheet with all the actions. The peek works on the farm overview too — tap or hover any status dot. In Select mode, taps keep picking cages like before.
8. Tap a stocked cage twice to log **growth checks**, **mortality**, or **harvest**. Growth checks can apply to the whole line. Each cage shows a growth chart and time in water.
9. **Worked a batch of cages? Select first, fill in once.** Inside a plot, tap **Select**, then tap each cage you worked (or a line's **All** button). The bottom bar gives three quick actions: **Empty** (dump the batches — cages stay on the line, ready to restock), **Remove** (take the cages off the line — each spot stays put as a dashed marker), and **Log** (one form for the whole selection: stock the empties, or growth check / mortality / harvest / note across the stocked ones — counts apply per cage, capped at what each cage has left, and "Full harvest" empties every selected cage in one go).
10. Colors on the map: gray = empty, green = under 6 months, amber = 6–12 months, orange = over 12 months, blue ring = at market size (default 76 mm, changeable in ⚙ settings).
11. **Back up regularly**: ⚙ menu → Export data. Keep the JSON file somewhere safe; Import restores it on any device.
